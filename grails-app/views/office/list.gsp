	...
	<body>
		...
		<div id="list-office" class="content scaffold-list" role="main">
			...
			<table>
				<thead>
					<tr>
						<g:sortableColumn property="level.level" title="${message(code: 'office.level.label', default:'Level')}" /></th>
				...
				<g:each in="${officeInstanceList}" status="i" var="officeInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						<td>${fieldValue(bean: officeInstance, field: "level.name")}</td>
						<td> ${officeInstance.children.size()} ${fieldValue(bean: officeInstance, field: "name")}</td>
